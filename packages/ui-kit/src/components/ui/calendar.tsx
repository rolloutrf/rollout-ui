...
import { Button as _Button, buttonVariants } from '@ui-kit/components/ui/button'

type ButtonComponent = typeof _Button
...
function Calendar({
  ...
  buttonVariant?: React.ComponentProps<ButtonComponent>['variant']
}) {
    ...
    <DayPicker
      ...
      components={{
        ...
        Chevron: ({ className: c, orientation, ...rest }) => {
          if (orientation === 'left') return <ChevronLeftIcon className={cn('size-4', c)} {...rest} />
          if (orientation === 'right') return <ChevronRightIcon className={cn('size-4', c)} {...rest} />
          return <ChevronDownIcon className={cn('size-4', c)} {...rest} />
        },
        DayButton: (dbProps) => <CalendarDayButton locale={locale} {...dbProps} />,
