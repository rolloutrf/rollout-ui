...
**Как генерировать правильно начиная с компонента**

**1. Самое главное - сгенерировать базовый компонент**

```
Создай компонент по следующему макету фигмы и размести его в packages/ui-features/src/features/Card
Назови компонент ItemCardDetail
Реализуй все элементы компонента по макету. Используй абстракции для текстов и прочих специфичных для указанного примера элементов.
Не используй специфичные для примера данные в качестве значений по умолчанию для пропсов компонентов и функций. Исключение - названия UI элементов, например текст внутри кнопок.
Сделай story для нового компонента.
Обязательно используй правила из AGENTS.md.
<https://www.figma.com/design/eiItTzL0Q2QFL5S8XQXFui/5.-ItemCard?node-id=1291-15272&t=V8FZGXl5EBne09C9-4>
```

Посмотреть результат можно на локальном машине, выполнив команду

```
pnpm storybook
```

Сторибук будет доступен по ссылке <http://localhost:6006>

**2. После того как сгенерировали компонент и сделали все правки.**
ИИ агент может пропустить что-то из правил, надо отдельно попросить его все исправить

```
Отредактируй новый компонент в соотвествии с правилами AGENT.md
```

**3. Добавь новый компонент в приложение-пример. Тут смотрите сами, куда его лучше приткнуть, главное указать, что компонент должен быть в apps/demo**

```
Добавь пример использования нового компонента на главную страницу в apps/demo
```

**4. Выполните команды, чтобы код был отформатирован по стандартам. Можно попросить ИИ агента, а не делать самому**
...
**5. Создайте новый Issue на ревью на гитхабе проекта**
...
1. Добавьте экспорт в `packages/ui-kit/src/index.ts`
