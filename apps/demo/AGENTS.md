...
> ⚠️ **Запускать pre-flight ПЕРЕД каждой задачей на вёрстку страницы.** Прохождение всех 5 проверок занимает ≤30 секунд. Цена пропуска — застревание в инфраструктуре посреди работы (реальные кейсы: Vite падал из-за Node 20 без нативного rolldown-биндинга; preview MCP подхватывал чужой `launch.json`; Figma возвращала 403 и приходилось перезапускать Claude Code посреди сессии).
>
> Если хотя бы одна проверка валится — остановиться и пройти её установочный шаг. Не «обходить» проблему по ходу.
